"""
===============================================================================
EXAMPLE 1 DEPLOYMENT & EXECUTION RUNNER
===============================================================================
Description:
    Packages lambda_function.py, creates the function via Boto3 on LocalStack 
    or AWS, triggers it with `sample_s3_event.json`, and prints returned outputs.
...
# =============================================================================
# SECTION 1: IMPORTS & UTF-8 TERMINAL ENCODING
# =============================================================================
import boto3
...
# Ensure terminal stdout handles unicode/emojis on Windows terminals
...
FUNCTION_NAME = "s3-thumbnail-generator-example"
ROLE_ARN = "arn:aws:iam::123456789012:role/lambda-s3-role"
RUNTIME = "python3.12"
HANDLER = "lambda_function.lambda_handler"

EVENT_FILE = os.path.join(os.path.dirname(__file__), "sample_s3_event.json")

# Configure short timeout for local testing fallback
BOTO_CONFIG = Config(connect_timeout=3, read_timeout=3, retries={'max_attempts': 1})

# =============================================================================
# SECTION 3: HELPER & MAIN FUNCTIONS
# =============================================================================
def create_zip_bytes():
    """Compresses lambda_function.py into in-memory zip bytes."""
...
def main():
    ...
    try:
        ...
        lambda_client.create_function(
            ...
            Description="Example 1: S3 File Upload Event Processor",
            Timeout=15,
            MemorySize=128
        )
        print("✅ Function created successfully!")
    except lambda_client.exceptions.ResourceConflictException:
        print(f"🔄 Function '{FUNCTION_NAME}' already exists. Updating code package...")
    ...
    # Load S3 Event Payload
    with open(EVENT_FILE, "r") as f:
        event_payload = json.load(f)

    # Invoke Function
    try:
        print("⚡ Invoking Lambda function with mock S3 upload event...")
